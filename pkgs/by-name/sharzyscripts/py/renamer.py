...
def main():
    ...
    parser.add_argument('--per-track-artist', action='store_true', help='add artist name in each track')
...
def handle(args, logger):
    ...
    for f in args.files:
        ...
        if not (p / 'cover.jpg').exists():
            ...
            for candidate in ('Cover.jpg', 'Folder.jpg', 'folder.jpg', 'front.jpg', 'Front.jpg'):
            ...
            if suffix in MUSIC_SUFFIXES:
                ...
                def update_meta(new_val, old_val, meta_name):
                    if old_val is not None and new_val != old_val:
                        print(Color.yellow(f'warning: inconsistent {meta_name} "{old_val}" and "{new_val}" ({child.name})'))
                        return old_val
                ...
                album_artist = update_meta(sanitize_name(cur_meta('albumartist')), album_artist, "album_artist")

                year = update_meta(cur_meta('year'), year, "album_artist")
                if year is not None and '.' in year:
                    logger.critical(f'year "{year}" in file "{child}" should not be separated by "."')

                totaldiscs = update_meta(int(cur_meta('totaldiscs')), totaldiscs, 'totaldiscs')
                label = update_meta(tags.raw.get('label', default=None), label, 'label')
                catalog_number = update_meta(tags.raw.get('catalognumber', default=None), catalog_number, 'catalog_number')
                ...
                bitlen = 16 if child.suffix == '.mp3' else get_meta(child, tags, '#bitspersample')
                bitlens.add(bitlen)
                ...
                encoding_info = f'{bitlen}bits x {sample_rate}Hz {int(bit_rate/1000)}kbps'
                meta_info = f'{Color.cyan(format_len(length))} {Color.green(album_artist)} {Color.gray(encoding_info)}'
                if child.name != new_name:
                    print(f'{Color.gray(relative_dir + "/")}{Color.blue(child.name)} -> {Color.red(new_name)} {meta_info}')
                    rename_plans.append((child, child.with_name(new_name)))
                else:
                    print(f'{Color.gray(relative_dir + "/")}{Color.blue(child.name)} {Color.gray("(unchanged)")} {meta_info}')
            elif suffix in OTHER_SUFFIXES:
                print(f'{Color.gray(relative_dir + "/")}{Color.blue(child.name)} {Color.gray("(non-music file, ignored)")}')
        ...
        format_str = '+'.join(sorted(list(formats)) + sorted(list(other_formats)) + assets)
