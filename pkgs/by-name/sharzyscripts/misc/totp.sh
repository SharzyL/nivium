...
TOTP_SERVER="$(echo "${TOTP_SERVER}" | tr "[:upper:]" "[:lower:]")"
...
if [ ! "${TOTP_INTERVAL}" -gt 0 ] 2>&-; then
  ...
  TOTP_INTERVAL="${TOTP_INTERVAL//^00*/}"
fi
...
gen_token() {
  ...
  case "${server}" in
    ...
    google | github | gitlab | bitbucket)
      local key
      key="$(echo "${secret}" | base32 -d | hex_pack)"
      [ -z "${key}" ] && exit 2
      ...
      local start
      start="$(( (0x$(echo "${digest}" | cut -b 40)) * 2 + 1))"
      [ "${start}" -gt 33 ] && exit 4
      # Read a 32-bit positive integer and take at most six rightmost digits.
      local hexes
      hexes="$(echo "${digest}" | cut -b "${start}-$(( start + 7 ))")"
      local token
      token="$(( ((0x${hexes}) & 0x7FFFFFFF) % 1000000 ))"
